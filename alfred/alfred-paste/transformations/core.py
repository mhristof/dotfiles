...
def base64_transform(content: str) -> str:
    ...
    if (base64_pattern.match(clean_content) and 
...
def update_workflow(content: str) -> str:
    ...
    try:
        ...
        if version_compare(latest_version, __version__) <= 0:
            return f"Already on latest version v{__version__} (latest: v{latest_version})"
        
        # Download the latest workflow file
        with tempfile.NamedTemporaryFile(suffix='.alfredworkflow', delete=False) as tmp_file:
